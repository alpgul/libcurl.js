...
## v0.8.13 (9/9/26):
- Client: dynamic curl option control. New typed JS→C bridge `set_curl_option(handle, "CURLOPT_TIMEOUT", 30)` (also `session.set_curl_option(...)` and the object form `{opt, type, value}`) exports the raw option to JS for the first time. A C-side allowlist (`client/libcurl/option.c`) maps option names to `curl_easy_setopt` calls; only scalar `long`, copied-string, and header-list options are reachable — no raw pointers, callbacks, or blob-based options whose memory would need to outlive the call (those stay on the canonical JSON `params` surface, which remains the primary API and is untouched). The bridge applies to a single per-request easy handle between `stream_response()` and `start_request()`. The allowlist covers timeouts, low-speed abort, redirects, buffer sizing, http version, resume, max filesize, and useragent/referer/range/custom-method/encoding/cookie/basic-auth strings, plus `CURLOPT_HTTPHEADER_APPEND` (newline-separated "Name: value" lines). Unknown options and invalid values throw. Wasm rebuilt via the emscripten/emsdk:3.1.72 docker builder (deps cached). Client bumped to 0.8.4
